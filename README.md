...
| Secciones › El cuarteto (`about`)         | Título, texto y foto del grupo.                                                                                                                                                                                                                                                       |
| Secciones › Repertorio / Contacto         | Los textos de cada sección (títulos, notas, estado vacío, mensaje tras enviar).                                                                                                                                                                                                       |
...
| Contenido › Integrantes (`members`)       | Los cuatro músicos, con orden de atril, instrumento, nombre corto, semblanza, retrato y encuadre.                                                                                                                                                                                     |
| Contenido › Presentaciones (`events`)     | Fechas con hora, ciudad y foto, más el detalle de la ventana (descripción, programa, lugar, precios y enlace de boletos). La portada muestra las dos próximas **publicadas**; una sin la casilla «Publicada» se guarda sin salir en el sitio, y las pasadas se quedan como historial. |
...
Los bloques sobre tinta (portada, integrantes, repertorio y pie) llevan una capa de grano por encima de todo su contenido (clase `ink-grain` en `app/(site)/globals.css`): el mismo tile grueso de 300 px sobre fondo, fotos y tarjetas. Las secciones sobre marfil van limpias. Las fotos llevan además su propio grano fino (`.grain`).

## Reproductor de repertorio

Todavía no hay grabaciones de estudio. El reproductor (`hooks/use-sample-player.hook.ts`) simula la reproducción: avanza un reloj sobre la duración de la obra, nunca arranca solo y sólo "suena" una pista a la vez; anterior y siguiente recorren la lista en círculo. Cuando existan los archivos de audio, ese hook es el único sitio que hay que cambiar; la interfaz ya está hecha: la tarjeta oscura de la obra (con la foto del grupo de «El cuarteto» como portada bajo una clave de sol, y el título en marquesina si no cabe, `hooks/use-marquee.hook.ts`), la barra de progreso en forma de pentagrama con notas que se iluminan al pasar, los filtros en píldoras, el programa con miniatura por obra y ecualizador, y un mini reproductor fijo al pie de la pantalla (mientras existe, el pie de página se rellena por debajo con su altura para que no tape nada) que aparece mientras una obra suena y la tarjeta ya no se ve (`hooks/use-in-view.hook.ts`); en el teléfono no hay tarjeta y la barra es todo el reproductor: aparece al elegir una obra y se queda mientras esté en curso, aunque esté en pausa; se retira hacia abajo en cuanto la tarjeta vuelve a verse y, si lo que pasa es que se pausa, espera cinco segundos antes de irse (`hooks/use-mini-player-presence.hook.ts`). Los rótulos fijos del reproductor («Sonando ahora» del mini reproductor, los `aria-label` de los controles) están en `PLAYER_COPY` de `constants/repertoire.const.ts`, no en el CMS.
